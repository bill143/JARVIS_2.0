...
Bound to this repository's ``Trace`` shape, which carries no ``messages`` field
(unlike the upstream reference).
"""
...
class TraceCollector:
    ...
    ``TraceStep`` objects.  When the agent finishes, the complete ``Trace`` is
    persisted to the ``TraceStore`` and published on the bus.
