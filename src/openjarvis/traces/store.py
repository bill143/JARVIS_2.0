...
First-party implementation bound to this repository's ``Trace`` / ``TraceStep`` /
``StepType`` / ``EventBus`` shapes. Unlike the upstream reference it carries no
``messages`` column and depends only on the standard library (``pathlib`` +
``sqlite3``) — no ``openjarvis.security.file_utils`` helper.
"""
...
class TraceStore:
    ...
    def __init__(self, db_path: str | Path) -> None:
        ...
        if raw == ":memory:":
            ...
            # Resolve ``~`` and ensure the parent directory exists (no external
            # ``secure_create`` helper — standard library only).
            path = Path(raw).expanduser()
            path.parent.mkdir(parents=True, exist_ok=True)
            self._db_path = str(path)
        # check_same_thread=False is safe with WAL mode. Agent work may be
        # dispatched to a thread pool, so trace writes can originate from a
        ...
        # FTS5 powers ``search``; if the SQLite build lacks it, degrade to a
        # LIKE scan rather than failing to open the store.
        self._has_fts = True
        try:
    ...
    def save(self, trace: Trace) -> None:
        ...
        ``trace_id`` is a primary key: saving a second, different trace under an
        existing id raises ``sqlite3.IntegrityError``.
        """
        self._conn.execute(
            _INSERT_TRACE,
            (
    ...
    def search(
        ...
        *,
        ...
        if self._has_fts:
            ...
            # Degraded path (no FTS5): substring match on query/result.
            sql = (
                "SELECT trace_id, query, result, agent, model, outcome, started_at "
                "FROM traces WHERE (query LIKE ? OR result LIKE ?)"
            )
            like = f"%{query}%"
            params = [like, like]
            if agent:
